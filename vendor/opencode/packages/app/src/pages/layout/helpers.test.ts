...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("meiling://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("meiling://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("meiling://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("meiling://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("meiling://open-project")).toBeUndefined()
    expect(parseDeepLink("meiling://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "meiling://open-project?directory=/a",
      "meiling://other?directory=/b",
      "meiling://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("meiling://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("meiling://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("meiling://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("meiling://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "meiling://new-session?directory=/a",
      "meiling://open-project?directory=/b",
      "meiling://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __MEILING__: {
        deepLinks: ["meiling://open-project?directory=/a"],
      },
    } as unknown as Window & { __MEILING__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["meiling://open-project?directory=/a"])
